/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/6350*@* */
...
//HashMap<String,ArrayList>aa= new HashMap<String,ArrayList>();
...
speedmult = 3*norm(mouseX,0,width);
  ...
  ArrayList treeKeys = new ArrayList(trees.keySet());//暂存数组
  Collections.sort(treeKeys);
  Iterator<Tree> tki =   treeKeys.iterator();

  while(tki.hasNext()){//画上排序后的树
